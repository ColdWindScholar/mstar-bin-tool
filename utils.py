...
def processMmc(line):
    ...
    # mmc create.complete - complete eMMC gp, enhance user, reliable write partition setting
        ...
        res = {'cmd': args[0], 'action': 'write.p.continue', 'addr': args[2], 'partition_name': args[3],
               'offset': args[4], 'size': args[5]}
        try:
        ...
        res = {'cmd': args[0], 'action': args[1], 'bootpart': args[2], 'addr': args[3], 'blk#': args[4],
               'size': args[5], 'partition_name': 'sboot'}
        try:
            res['empty_skip'] = args[6]
        except IndexError:
            res['empty_skip'] = 0
...
def generateFileName(outputDirectory, part, ext):
    ...
    if os.path.exists(fileName):
        ...
        fileName = os.path.join(outputDirectory,
                                part['partition_name'] + str(fileNameCounter[part['partition_name']]) + ext)
    return fileName
...
def generateFileNameSparse(outputDirectory, part):
    ...
    if os.path.exists(fileName):
        try:
            ...
            fileExtCounter[part['partition_name']] = 1
        fileName = os.path.join(outputDirectory,
                                part['partition_name'] + '_sparse.' + str(fileExtCounter[part['partition_name']]))
    return fileName
